---
title: "Complete Guide to Automatic Prompt Optimization in Python [wip]"
date: 2025-07-27
...
Wow! We went from 2/10 to 3/10. Progress!
...
Let's apply and evaluate the V2 prompt.
...
t.add_computed_column(extraction_v2 = extract_totals_v2_udf(t.invoice_image))
t.add_computed_column(is_same_v2 = metric_udf(t.ground_truth, t.extraction_v2))
```

```{python}
t.select(pxt.functions.mean(t.is_same_v2)).collect()
```

We still have 3/10, but not the exact same set of invoices a right now

```{python}
t.select(t.is_same, t.is_same_v1, t.is_same_v2).show()
```

All together Llama 4 Scout was able to get it righ 5 out of 10 times, so there is probably a prompt out there the could achieve at least those results. We will stop here for the manual optimization. And move on to dspy as it is easier to iterate on and we will try to furter improve llama's performance.


### Recap of Manual Optimization

We successfully built a simple, prompt optimizer by directly implementing a simplified version of the instruction generation strategy found in DSPy. The key takeaways are:

1.  **Grounded Generation**: We are grounding the prompt optimization generation in data. By analyzing specific failures and the goal, the system can self-improve.
2.  **The Power of Meta-Prompts**: Using an LLM to optimize the prompt of another LLM is effective.
3.  **Iterative Improvement**: Optimization is rarely a one-shot process. The iterative loop of Generate -> Evaluate -> Analyze Failures -> Regenerate will make us climb the hill.

While this manual implementation 'worked', it has limitations:
...
*   **It's Simplified**: We only optimized the instructions based on failures. The full MIPROv2 also optimizes few-shot examples, automatically summarizes the dataset and program code, uses history, and leverages randomized tips to inform future proposals (and more!).
*   **Simple Search Strategy**: We used a simple for loop. MIPROv2 and many other DSPy optimizers uses Bayesian optimization to search the space of possible prompts much more efficiently.
...
Finally we are in dspy, the code will be much simpler! First, let's create a connection. DSPy uses litellm behind the scene so the model string that worked above are the same here. But there is much less boilerplate. For instance, we can create a connection to llama and call it just like that:
...
As we discussed above, to optimize a task we need the optimizers to know what our intent is. We mentionned 4 ways to specify intent: 1) by example, with a goldset, 2) by a rubric or judge, 3) by instructions and/or 4) by stating the input/output. Here we will provide example (1), instructions (3), and state the required inputs and outputs (4).

As a reminder out instruction were:

> Extract the after-tax total and the before-tax total from the invoice.

So in dspy, to provide the input, ouptut requirements and the instructions we write a signature. To write a signature we do that:
...
class OurIntent(dspy.Signature): #OurIntent can be any name we want
    ...
    invoice_totals: InvoiceTotals = dspy.OutputField()
```

That's it, see how we reuse the pydantic model `InvoiceTotals` we created above. Now let's try it out.

First we must turn our signature into what dspy call a 'program' and set the language model to use:
...
Now we can call our program:

```{python}
dprogram(invoice_image = dspy.Image.from_file(file_path = "images/invoices/IMG_2160.jpg"))
```

That program contains information about the instructions, input/outputs, etc. It uses templates called adapters to turn those into a prompt. At this point in our process dspy has not optimized anything for us it have simply helped us focus only on the part that matters. It was much simpler to get to an llm call then it was before. Let's now add the base dspy prompt into our pixel table.


```{python}
@pxt.udf
def dspy_baseline(img: Image.Image) -> Dict[str, float]:
    return dprogram(invoice_image = dspy.Image.from_PIL(img)).invoice_totals.model_dump()

t.add_computed_column(extraction_dspybase = dspy_baseline(t.invoice_image))
```
...
```{python}
t.select(pxt.functions.mean(t.is_same_dspybase)).collect()
```

Here, with simple dspy we are also starting at 2/10. So not worse because of the dspy template but not better either.
...
## DSPy Instruction Optimization

As mentionned previously, dspy can optimize many components of the AI system, now we will focus on optimizing only the instruction, similarly to what we have just done 'manually'.

The optimizer will need, a trainset, a metric, and a program. We aleary how the program, now we need to reshape our 10 example in dspy object. DSPy wants a list of `dspy.Example` objects. Similarly we will need to adapt the metric to be using the `dspy.Example` object.

### Adjusting for DSPy

Let's reuse the list of dictionnay that use used at the beginning of the tutorial and reshape it for dspy.
...
trainset = [dspy.Example(
    invoice_image = dspy.Image.from_file(i["invoice_path"]),
    ...
    ).with_inputs("invoice_image") for i in goldset
]
```

now for the metric, this is what we have beend working with so far:
...
and this is what dspy expects: 

```{python}
def metric_dspy(example, pred, trace=None):
    is_btax_same = example.invoice_totals.before_tax_total == pred.invoice_totals.before_tax_total
    is_atax_same = example.invoice_totals.after_tax_total == pred.invoice_totals.after_tax_total
    return float(is_btax_same and is_atax_same)
```
...
teacherp.set_lm(lm = dspy.LM("openrouter/google/gemini-2.5-pro"))
optimizer = dspy.MIPROv2(
    metric_dspy, 
    max_bootstrapped_demos = 0, 
    max_labeled_demos = 0, 
    prompt_model=dspy.LM("openrouter/google/gemini-2.5-pro"), 
    auto = "medium")
dprogram_optimized = optimizer.compile(dprogram, trainset=trainset, requires_permission_to_run = False, teacher = teacherp)
```
...
```{python}
dprogram_optimized(invoice_image = dspy.Image.from_file(file_path = "images/invoices/IMG_2160.jpg"))
```
...
def dspy_optimized(img: Image.Image) -> Dict[str, float]:
    return dprogram_optimized(invoice_image = dspy.Image.from_PIL(img)).invoice_totals.model_dump()

t.add_computed_column(extraction_dspyopt = dspy_optimized(t.invoice_image))
```
...
```{python}
t.select(pxt.functions.mean(t.is_same_dspyopt)).collect()
```
...
6/10! We are improving!

#### Let's now add few-shots examples.